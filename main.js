...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function animateOnScroll() {
    cards.forEach(function(card) {
      // Paksa kartu pertama untuk terlihat segera
      if (card.querySelector('h3') && card.querySelector('h3').textContent.includes('Bismillahirrahmanirrahim')) {
        card.classList.add('animated');
        card.style.opacity = '1';
        card.style.transform = 'translateY(0)';
      }
      // Pemeriksaan visibilitas normal untuk kartu lainnya
      else if (isElementInViewport(card) && !card.classList.contains('animated')) {
        card.classList.add('animated');
        card.style.opacity = '1';
        card.style.transform = 'translateY(0)';
      }
